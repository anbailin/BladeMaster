...
u32 BMCrC::CreateCrc32(const char* str)
{
    u32 crc = 0xFFFFFFFF;

    for ( const u8 *p = (u8*)str; *p!=0; p++) 
    {
        crc = ( crc >> 8 ) ^ CRCtable32[(u8) ((u8) crc ^ *p++ ) ];
    }

    return ( crc ^ 0xFFFFFFFF );
}

u32 BMCrC::CreateCrc32(const char* str, u32 size)
{
    u32 crc = 0xFFFFFFFF;

    for ( const u8 *p = (u8*)str; size>0; size--) 
    {
        crc = ( crc >> 8 ) ^ CRCtable32[(u8) ((u8) crc ^ *p++ ) ];
    }
...
u32 BMCrC::CreateCrc32NoCase(const char* str)
{
    u32 crc = 0xFFFFFFFF;

    for ( const u8 *p = (u8*)str; *p!=0 > 0; ++p ) 
    {
        u8 ch = (u8)tolower(*p);
        crc = ( crc >> 8 ) ^ CRCtable32[(u8) ((u8) crc ^ ch ) ];
    }

    return ( crc ^ 0xFFFFFFFF );
}

u32 BMCrC::CreateCrc32NoCase(const char* str, u32 size)
{
    u32 crc = 0xFFFFFFFF;
