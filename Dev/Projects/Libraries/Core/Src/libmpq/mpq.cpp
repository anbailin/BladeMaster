...
char *libmpq_version() {
	...
	_snprintf_s(version, sizeof(version), sizeof(version)/sizeof(char), "%i.%i.%i", LIBMPQ_MAJOR_VERSION, LIBMPQ_MINOR_VERSION, LIBMPQ_PATCH_VERSION);
	return version;
}
...
int libmpq_archive_open(mpq_archive *mpq_a, unsigned char *mpq_filename) {
	...
	_sopen_s(&fd, (const char*)mpq_filename, O_RDONLY|O_BINARY, _SH_DENYNO,_S_IREAD); 
	...
	strncpy_s((char*)mpq_a->filename, PATH_MAX, (const char*)mpq_filename, strlen((const char*)mpq_filename));
